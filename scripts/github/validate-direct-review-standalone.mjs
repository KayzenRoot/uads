...
function validate(item) {
  ...
  if (item.version === "0.11.0") for (const key of ["linux", "windows"]) if (item.compatibility?.[key]?.outcome !== "success" || item.compatibility?.[key]?.commitSha !== item.commitSha) errors.push("compatibility-not-proven:" + key);
