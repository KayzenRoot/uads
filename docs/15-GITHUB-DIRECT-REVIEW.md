...
`.github/workflows/compatibility.yml` runs the bounded Node 20 Linux/Windows matrix after the source CI run and publishes per-platform status artifacts. `.github/workflows/direct-review.yml` is triggered with `workflow_run` after that compatibility workflow completes. Its privileged publisher locates exactly one successful source CI run for the same SHA, checks out the exact source SHA, requires exactly one receipt artifact, queries the source run and Foundation job/steps, verifies both compatibility jobs, rejects mismatched or ambiguous identity, and emits the canonical `github-direct-review-evidence.json` between the stable markers `UADS_DIRECT_REVIEW_BEGIN` and `UADS_DIRECT_REVIEW_END`. A failed source CI or missing compatibility proof remains explicit failure/incompleteness; it is never converted to PASS.
