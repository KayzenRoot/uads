...
Negotiation is an intersection: a profile capability is usable only when the runtime snapshot proves the same capability. `unknown` is conservative false. The router applies the Work Order quality floor before deterministic scoring, rejects insufficient context or output reserve, and sorts by minimum sufficient class, reasoning adequacy, context headroom, relative cost, relative latency, then `profileId`. Unknown cost is not treated as cheapest. Critical and other assurance-sensitive work fail closed when no sufficient proven profile exists; an empty registry is host-managed compatibility only for non-critical economy/balanced work and is explicitly recorded as such.
